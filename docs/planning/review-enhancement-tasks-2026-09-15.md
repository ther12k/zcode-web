...
| ZWUI-067 | Done — git status words (server + UI), true run counts, truthful status copy, pinned reachable beyond latest-50, hidden-session restore in Settings, plus three live-QA fixes: context chip shows CURRENT CONTEXT (step-finish totals are per-step billing, not size — 463k steps summed to ~95B on this store), sidebar row actions no longer overlap (Pin used to be intercepted by the absolutely-positioned Hide), and pinned rows carry an unpin control. scripts/live-qa-sweep.mjs verifies all of it against the running deployment (15/15) |
...
| ZWUI-070 | Hardened — Lugas now uses the modern Vite SPA shell, realpath-aware root/session/upload containment, session-context mismatch responses, attachment cap/collision-safe uploads, and Bun helper contract coverage (4 tests); full route/SSE contract suite remains pending and Node is still the supported backend |
...
| ZWUI-076 | Done — post-075 review fixes and polish: session loaders are identity-stable again (a locally attached run no longer refetches/flashes the transcript on phase changes), adoption records clear when the server stops reporting a job and never race an in-flight submission (which previously poisoned the replay cursor and rendered the previous job's output), and document.title shows a live "● N running" background-tab signal; unit + browser coverage added |
| ZWUI-077 | Done — desktop goal card is live for real: the session detail route now carries `workedMs` (turn_usage sum; 0 for older stores, never an error) and the goal seen in detail fetches reaches the Overview panel (list rows don't carry goals). The card shows the real objective, "In progress" state, live-ticking time while a run is busy, and tokens/updated meta when expanded; the chat context strip gains the desktop's session-level "Worked for" chip (completed turns + live elapsed). Browser coverage seeds real session_target/turn_usage rows; browser suite made single-worker to kill documented parallel-flake class |
| ZWUI-078 | Done — desktop Progress list: the CLI store's `todo` table (session-scoped checklist with pending/in_progress/completed) is exposed via `SessionStore.todos()` and the session detail route, flows to the Overview panel, and renders as a Progress section with a done/total badge, per-item status icons (check, spinner, empty circle), strike-through for completed rows, and live spinner while a run is busy; empty for sessions that never created todos and tolerant of stores without the table. Server + browser coverage added |
| ZWUI-079 | Done — topbar "Worked for" line: the desktop shows cumulative agent working time under the session title; the web topbar now stacks title + a live "Worked for" line (turn_usage sum + current busy period, 1s tick). Shared `formatDuration` exported from ui (was duplicated privately in two components); topbar row height matches the desktop's two-line header on wide screens, line hidden ≤1280px where the header is single-line. Plans section from the desktop was investigated and is NOT surfaced: the store's workflow tables are empty and no plan documents are persisted — there is no honest data source yet |
