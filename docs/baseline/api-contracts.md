...
| GET /api/sessions/:id?limit&offset | — | `{session{...goal}, transcript:[{role,text,tools?[],files?[]}], total, hasMore, runActive, runStartedAt, activeJobId, workedMs}` | logical-message pagination; offset counts from newest; `activeJobId` is present only for an in-flight job owned by this server instance, including a just-created session before persistence catches up; `workedMs` is the completed-turn working-time sum (0 when the store lacks turn_usage) |
